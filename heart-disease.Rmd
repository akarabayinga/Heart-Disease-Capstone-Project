...
Machine learning (ML), a scientific discipline which uses computer algorithms that learn and adapt, can be exploited to diagnose a number of medical disorders [@sajda2006machine; @deo2015machine]<!--# Make sure these are in a pair of brackets) -->. ML approaches include *supervised learning*, which entails using labeled data to train algorithms to classify data or predict outcomes, and *unsupervised learning*, which aims to identify naturally occurring patterns and trends or clusters data into groups using unlabelled data [@deo2015machine].
...
-   [Predictor variables]{.underline}: <!--# Add ranges of numeric variables? -->
...
Table \@ref(tab:descriptive-statistics) shows the descriptive statistics of all predictor variables stratified by HD status. HD appears to be associated with <!--# For Dimples: This is the overview based on the descriptive stats table, the insights gained is following deeper EDA-->:
...
<!--# For graphs, order from lowest HD present percent to highest; check that x-axis labels are visible-->
